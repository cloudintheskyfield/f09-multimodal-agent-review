VOICE_SEPARATION_SPEC_DETERMINISTIC

【定位】
- version：2.1.0
- phase：Bound Node -> Node Rewriter -> Prompt Validation
- prompt family：voice_separation
...
- compatible routes：由 Capability Catalog / Binding Registry 映射到 voice_separation family 的 route；本 Prompt 不维护或选择 Rxx
...

{
  ...
  "prompt_call": {
    "prompt_id": "VOICE_SEPARATION_SPEC_DETERMINISTIC",
    "prompt_version": "2.1.0"
  },
  "node": {
    "node_id": "n02",
    "route_id": "R09",
    "objective": "从双声道音频分离主唱并保留环境感",
    "expected_outputs": [
      {
        "key": "separated_audio",
        "type": "audio",
  ...
  "resolved_context": {
    ...
    "context_facts": [
      "Runtime 安全输出文件名：separated_vocals.wav"
    ]
  },
  "assets": [
    {
  ...
  "upstream_outputs": [],
  "binding": {
    "binding_id": "binding-voice_separation-001",
    "prompt_profile": {
      "profile_version": "voice_separation_prompt_profile.v1",
...
【能力专属规则】
1. 这是 deterministic `operation_spec`，必须有一个 `source_audio`；缺失时 blocked。
2. 明确 target_stem、输入/输出 channels、sample_rate、denoise、ambience policy、music bleed policy、output format、output naming 与 failure conditions。
3. 不声称已经听取音频；只有 metadata 或 evidence-linked analysis 可支持声道、采样率、噪声和串音事实。
4. 不创建 profile 未支持的 stem，不偷偷降采样、合并声道或改变时长；硬要求不支持时 blocked。
5. 输出命名不得泄露路径、用户信息或 Provider 元数据；只使用 Runtime 提供的安全 basename/template。
6. 结果验收关注目标 stem、串音、过度处理、金属音、呼吸截断和格式一致性，但本 Prompt 不执行最终质量评分。
...

{
  ...
  "prompt": {
    ...
    "content": "OPERATION: voice_separation\nSOURCE: SOURCE_AUDIO_0\nTARGET_STEM: vocals\nINPUT_CHANNELS: 2\nOUTPUT_CHANNELS: 2\nSAMPLE_RATE_HZ: 48000\nDENOISE: conservative\nPRESERVE_AMBIENCE: true\nMUSIC_BLEED_POLICY: minimize without metallic artifacts\nOUTPUT_FORMAT: wav\nOUTPUT_NAME: separated_vocals.wav\nFAILURE_CONDITIONS: unsupported stem, unreadable source, channel mismatch, or inability to preserve required sample rate"
  },
  ...
  "validation": {
    ...
    "forbidden_concepts": [
      "sample-rate downgrade",
      "channel merge"
    ],
    "timeline_duration_ms": null,
    "prompt_char_count": 405
  },
  "provenance": {
    "rewriter_id": "VOICE_SEPARATION_SPEC_DETERMINISTIC",
...
- provenance.rewriter_id 固定为 VOICE_SEPARATION_SPEC_DETERMINISTIC，rewriter_version 固定为 2.1.0，source 固定为 deterministic；template_refs 仅放 Runtime 实际注入的内部 pattern ID。
