TEXT_TO_VIDEO_PROMPT_DETERMINISTIC_FALLBACK

【定位】
- version：3.1.0
- phase：Bound Node -> Node Rewriter -> Prompt Validation
- prompt family：text_to_video
...
- compatible routes：由 Capability Catalog / Binding Registry 映射到 text_to_video family 的 route；本 Prompt 不维护或选择 Rxx
...

{
  ...
  "prompt_call": {
    "prompt_id": "TEXT_TO_VIDEO_PROMPT_DETERMINISTIC_FALLBACK",
    "prompt_version": "3.1.0"
  },
  "node": {
    "node_id": "n02",
    "route_id": "R05",
    "objective": "生成 8 秒竖屏咖啡产品预热视频",
    "expected_outputs": [
      {
        "key": "video",
        "type": "video",
  ...
  "upstream_outputs": [],
  "binding": {
    "binding_id": "binding-text_to_video-001",
    "prompt_profile": {
      "profile_version": "text_to_video_prompt_profile.v1",
...
【能力专属规则】
1. 这是文本直接生成视频，不得要求或伪造首帧 source；若硬要求依赖 source frame，应 blocked。
2. 单镜头短动作可用 `text`；多镜头、多阶段或精确时序用 `timeline_text`，正文包含 GLOBAL CONTINUITY、连续 TIMELINE、ENDING、必要的 AVOID。
3. timeline 从 0 开始、无重叠、无空档、不超时，最后结束点等于参数总时长；毫秒总长写入 `validation.timeline_duration_ms`。
4. 每段只放一个主动作和主镜头意图，明确主体/环境运动路径、方向、速度、节奏、转场与连续性。
5. profile 不支持精确 timeline 时只能写顺序镜头；若精确时长是硬要求则 blocked。
6. 只有需求明确且 profile 支持 native audio 时才写音频；否则不得承诺同步音效、对白或口型。
...

{
  ...
  "prompt": {
    ...
    "content": "GLOBAL CONTINUITY\nPremium vertical coffee teaser; one matte-black cup; warm amber palette; no readable text.\n\n0-3000ms\nCoffee beans settle on a dark surface; slow forward camera push.\n\n3000-6500ms\nEspresso pours into the same cup; stable close camera; natural liquid motion.\n\n6500-8000ms\nCamera pulls back and stops; keep a clean upper-center logo-safe area.\n\nENDING\nHold a stable final frame.\n\nAVOID\nExtra cups, abrupt cuts, warped liquid, camera shake."
  },
  ...
  "validation": {
    "required_concepts": [
      "one consistent matte-black cup",
      "clean logo-safe area"
    ],
    "preservation_checks": [],
    "forbidden_concepts": [
      "readable brand text",
    ...
    "prompt_char_count": 454
  },
  "provenance": {
    "rewriter_id": "TEXT_TO_VIDEO_PROMPT_DETERMINISTIC_FALLBACK",
...
- provenance.rewriter_id 固定为 TEXT_TO_VIDEO_PROMPT_DETERMINISTIC_FALLBACK，rewriter_version 固定为 3.1.0，source 固定为 deterministic；template_refs 仅放 Runtime 实际注入的内部 pattern ID。
